/** 
 * All credit for the code in this class goes to the authors of the DND5e.
...
function cleanPackEntry(data, { clearSourceId=true, ownership=0 }={}) {
  ...
  if ( data._stats?.lastModifiedBy ) data._stats.lastModifiedBy = "ffxivttrpgbuilde";
...
async function extractPacks(packName, entryName) {
  ...
  for ( const packInfo of packs ) {
    ...
    logger.info(`Extracting pack ${packInfo.path}`);
    ...
    logger.info(`${JSON.stringify(folders)}`)
    logger.info(`${JSON.stringify(containers)}`)
