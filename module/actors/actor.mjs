...
export class FFXIVActor extends Actor {
  ...
  async _getEnmitySourceActor(effect) {
    const sourceActorUuid = String(
      foundry.utils.getProperty(effect, "flags.ffxiv.enmity.sourceActorUuid") ?? "",
    ).trim();
    const origin = sourceActorUuid || String(effect?.origin ?? "").trim();
  ...
  async _getEnmityCheckPenaltyInfo() {
    for (const effect of this._getEnmityEffects()) {
      ...
      if (sourceActor && !this._targetsIncludeActor(sourceActor)) {
        return {
          penalty: this._getEnmityEffectCheckPenalty(effect),
          sourceActor,
        };
      }
    }
  ...
  _getEnmityEffectCheckPenalty(effect) {
    const rawPenalty = foundry.utils.getProperty(
      effect,
    ...
    if (
      ...
      String(rawPenalty).trim() === ""
    )
      return -5;
    const effectPenalty = Number(rawPenalty);
    return Number.isFinite(effectPenalty) ? effectPenalty : -5;
  }
